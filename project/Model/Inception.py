...
class InceptionB(nn.Module):
    ...
    def _forward(self, x: Tensor) -> List[Tensor]:
        ...
        branch_pool = F.max_pool3d(x, kernel_size=3, stride=2, padding=1)
...
class InceptionD(nn.Module):
    ...
    def __init__(
        ...
        self.branch3x3_2 = conv_block(192, 320, kernel_size=3, stride=2, padding=1)
        ...
        self.branch7x7x3_5 = conv_block(192, 192, kernel_size=3, stride=2, padding=1)
...
class Inception3(nn.Module):
    def __init__(
        ...
        self.Conv3d_1a_3x3 = conv_block(3, 32, kernel_size=3, stride=2)
        self.Conv3d_2a_3x3 = conv_block(32, 32, kernel_size=3, padding=1)
        ...
        self.Conv3d_4a_3x3 = conv_block(80, 192, kernel_size=3, padding=1)
    ...
    def _forward(self, x: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        # N x 3 x 96 x 96 x 96
        x = self.Conv3d_1a_3x3(x)
        # N x 32 x 48 x 48 x 48
        x = self.Conv3d_2a_3x3(x)
        # N x 32 x 48 x 48 x 48
        x = self.Conv3d_2b_3x3(x)
        # N x 64 x 48 x 48 x 48
        x = self.maxpool1(x)
        # N x 64 x 24 x 24 x 24
        x = self.Conv3d_3b_1x1(x)
        # N x 80 x 24 x 24 x 24
        x = self.Conv3d_4a_3x3(x)
        # N x 192 x 24 x 24 x 24
        x = self.maxpool2(x)
        # N x 192 x 24 x 24 x 24
        x = self.Mixed_5b(x)
        # N x 256 x 12 x 12 x 12
        x = self.Mixed_5c(x)
        # N x 288 x 12 x 12 x 12
        x = self.Mixed_5d(x)
        # N x 288 x 12 x 12 x 12
        x = self.Mixed_6a(x)
        # N x 768 x 6 x 6 x 6
        ...
        # N x 768 x 6 x 6 x 6
        x = self.Mixed_7a(x)
        # N x 1280 x 3 x 3 x 3
        x = self.Mixed_7b(x)
        # N x 2816 x 3 x 3 x 3
        x = self.Mixed_7c(x)
        # N x 2816 x 3 x 3 x 3
        # Adaptive average pooling
        x = self.avgpool(x)
        # N x 2816 x 1 x 1 x 1
        x = self.dropout(x)
        # N x 2816 x 1 x 1 x 1
        x = torch.flatten(x, 1)
        # N x 2816
